...
# Patch kubelet to disable /etc/hosts management (DooD workaround)
...
RUN --mount=type=cache,id=go-${CRI_DOCKERD_VERSION},target=/go \
...
RUN --mount=type=cache,id=go-${CNI_VERSION},target=/go \
